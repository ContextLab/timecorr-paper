...
%DIF DEL old.tex    Fri Nov 20 13:32:41 2020
%DIF ADD main.tex   Fri Nov 20 15:41:30 2020
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{hyperref}
\usepackage{setspace}
...
%DIF < \newcommand{\rest}{S4}
%DIF -------
\newcommand{\kernelshape}{S1} %DIF > 
\newcommand{\kernelwidth}{S2} %DIF > 
\newcommand{\intact}{S3} %DIF > 
\newcommand{\para}{S4} %DIF > 
\newcommand{\word}{S5} %DIF > 
\newcommand{\rest}{S6} %DIF > 
\newcommand{\pca}{S7} %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{cognition-- }\DIFdelend \DIFaddbegin \DIFadd{cognition }\DIFaddend and, by extension, which types of neural activity patterns
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Neural patterns.  a. A space of neural features.}
    ...
    characterize the patterns of \DIFdelbeginFL \DIFdelFL{activities }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{activity }\DIFaddendFL across units.  Order 0
...
follows related approaches taken by][]{McInJirs19, TokeSomm19,
  GonzEtal19}; and a second non-invertible algorithm \DIFdelbegin \DIFdel{that explored
}\DIFdelend \DIFaddbegin \DIFadd{for computing
dynamic }\DIFaddend patterns in eigenvector centrality~\citep{Land95}.  This
latter approach characterizes correlations between each feature
dimension's relative \textit{position} in the network \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{(at each moment
in time) in }\DIFaddend favor of the specific activity histories of different
...
\section*{Results}
We sought to understand whether high-level cognition is \DIFdelbegin \DIFdel{supported by
}\DIFdelend \DIFaddbegin \DIFadd{reflected in
}\DIFaddend dynamic patterns of high-order correlations.  To that end, we
...
in several synthetic datasets (see \textit{Synthetic data\DIFaddbegin \DIFadd{: simulating
  dynamic first-order correlations}\DIFaddend } \DIFaddbegin \DIFadd{and }\textit{\DIFadd{Synthetic data:
...
participants listened to an \DIFdelbegin \DIFdel{auditorally }\DIFdelend \DIFaddbegin \DIFadd{auditorily }\DIFaddend presented story, listened to a
...
\subsection*{Recovering known dynamic correlations from synthetic
  data}
\DIFaddbegin \subsubsection*{\DIFadd{Recovering dynamic first-order correlations}}
\DIFaddend We generated synthetic datasets that differed in how the underlying \DIFaddbegin \DIFadd{first-order
}\DIFaddend correlations changed over time.  For each dataset, we applied
...
\begin{figure}[tp]
  ...
  \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Recovering known dynamic correlations from
      synthetic data.}}  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Recovering known dynamic first-order correlations
      from synthetic data.}}  \DIFaddendFL Each panel displays the average
    ...
    see \DIFdelbeginFL \textit{\DIFdelFL{Synthetic data}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{Synthetic data: simulating dynamic first-order
    ...
    correlation matrices unique to each event.  \DIFaddbeginFL \DIFaddFL{The vertical lines
    denote event boundaries.}\DIFaddendFL }
  \label{fig:synthetic}
\end{figure}
...
\DIFaddbegin \subsubsection*{\DIFadd{Recovering dynamic higher-order correlations}}
...
\begin{figure}[tp]
  ...
  \caption{\textbf{\DIFaddFL{Recovery of simulated first-order and second-order dynamic correlations.}}  \DIFaddFL{Each panel displays the average correlations
    ...
    see }\textit{\DIFaddFL{Synthetic data: simulating dynamic higher-order
      correlations}}\DIFaddFL{.  All estimates represented in this figure were
    computed using a Laplace kernel
    (width~$=20$).  }\textbf{\DIFaddFL{a. Constant correlations.}}  \DIFaddFL{These datasets
    have stable (unchanging) underlying second-order correlation
    matrices.  }\textbf{\DIFaddFL{b. Random correlations.}} \DIFaddFL{These datasets are
    generated using a new independently drawn second-order correlation
    matrix at each timepoint.  }\textbf{\DIFaddFL{c. Ramping correlations.}}
    \DIFaddFL{These datasets are generated by smoothly varying the underlying
    second-order correlations between the randomly drawn correlation
    matrices at the first and last timepoints.  }\textbf{\DIFaddFL{d. Event-based
      correlations.}}  \DIFaddFL{These datasets are each generated using five
...
eyes open (blinking as needed).  This \DIFaddbegin \DIFadd{public }\DIFaddend dataset provided a convenient
...
\begin{figure}[tp]
  ...
  \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Across-participant decoding accuracy varies with
      correlation order and cognitive engagement.}}
    %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Across-participant timepoint decoding accuracy varies with
      correlation order and cognitive engagement.}}
    \DIFaddendFL \textbf{a.~Decoding accuracy as a function of order: PCA.}
    ...
    word: $\frac{1}{300}$; rest: $\frac{1}{400}$\DIFaddbeginFL \DIFaddFL{; these chance accuracies
    were subtracted from the observed accuracies to obtain the
    relative accuracies reported on the $y$-axis}\DIFaddendFL ).  The error ribbons
    ...
    denote sets of features that yielded reliably higher (\DIFdelbeginFL \DIFdelFL{upwards
    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{upward
    }\DIFaddendFL facing) or lower (downward facing) decoding accuracy than the mean
    of all other features (via a two-tailed \DIFdelbeginFL \DIFdelFL{test}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t$-test}\DIFaddendFL , thresholded at
    ...
    \DIFdelbeginFL \textbf{\DIFdelFL{b.~Normalized decoding accuracy as a function of order:
      PCA.}}  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{b.~Normalized timepoint decoding accuracy as a function of order:
      PCA.}}  \DIFaddendFL This panel displays the same results as Panel a, but here
    ...
    a lower-dimensional space.  \DIFdelbeginFL \textbf{\DIFdelFL{c.~Decoding accuracy as a
      function of order: eigenvector centrality.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{c.~Timepoint decoding accuracy as a
      function of order: eigenvector centrality.}} \DIFaddendFL This panel is in the
    ...
    \DIFdelbeginFL \textbf{\DIFdelFL{d.~Normalized decoding accuracy as a function of order:
      eigenvector centrality.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{d.~Normalized timepoint decoding accuracy as a function of order:
      eigenvector centrality.}} \DIFaddendFL This panel is in the same format as
    ...
    lower-dimensional space. \DIFaddbeginFL \DIFaddFL{See Figures~\kernelshape~and~\kernelwidth~for decoding results broken down by kernel shape and width, respectively.}\DIFaddendFL }
  \label{fig:decoding}
\end{figure}
...
$n^\mathrm{th}$-order dynamic correlations (see \textit{Feature
  weighting and testing}\DIFaddbegin \DIFadd{, Fig.~\ref{fig:pipeline}}\DIFaddend ).  We repeated this process for
...
(\DIFaddbegin \DIFadd{represented by }\DIFaddend a $T$ by
$\mathcal{O}(K^2)$ matrix) onto a lower-dimensional \DIFaddbegin \DIFadd{feature space
(represented by a }\DIFaddend $T$ by $K$ matrix\DIFaddbegin \DIFadd{)}\DIFaddend .
...
\caption{\textbf{Statistical summary of decoding accuracies for
    different neural features.}  Each column \DIFaddbeginFL \DIFaddFL{of matrices }\DIFaddendFL displays decoding
  ...
  In the upper triangles of each \DIFdelbeginFL \DIFdelFL{map}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{matrix}\DIFaddendFL , warmer colors (positive $t$-values) indicate that the
  ...
  entries display the relative average optimized weight given to each type of feature \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL in
...
that\DIFaddbegin \DIFadd{, }\DIFaddend while the resting state times could be decoded reliably, the
...
\begin{figure}[tp]
  ...
  \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Top terms associated with the endpoints of the
      strongest correlations.}}  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Top terms associated with the most strongly
      correlated nodes at each order.}}  \DIFaddendFL Each color corresponds to one order of
    inter-subject functional correlations.  \DIFaddbeginFL \DIFaddFL{To calculate the dynamic
    ...
    features of the original space. }\DIFaddendFL The inflated brain plots
...
accuracies that \DIFdelbegin \DIFdel{best matched the }\DIFdelend \DIFaddbegin \DIFadd{were the most strongly correlated (across conditions
and orders) with the }\DIFaddend average accuracies across all of the
...
of 50 (Fig.~\ref{fig:kernels}d\DIFaddbegin \DIFadd{; also see Fig.~\pca}\DIFaddend ).  We used this kernel to compute a
...
word\DIFaddbegin \DIFadd{; top three rows of Fig.~\ref{fig:neurosynth}}\DIFaddend ), we found that
...
the ventrolateral prefrontal cortex.  However, \DIFdelbegin \DIFdel{during
listening }\DIFdelend \DIFaddbegin \DIFadd{when participants
listened }\DIFaddend to temporally scrambled stories, these higher-order
correlations instead involved interactions with additional regions
associated with speech and semantic processing \DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{(second and third rows
of Fig.~\ref{fig:neurosynth}).  }\DIFaddend By contrast, we found
a much different set of patterns in the resting state data
\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(Fig.~\ref{fig:neurosynth}, bottom row).
}\DIFaddend First-order resting state correlations were most strongly associated
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Proposed high-order network dynamics underlying
      high-level cognition during story listening.}  \DIFdelbeginFL \DIFdelFL{Higher }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Schematic depicts
    higher }\DIFaddendFL orders of
    network interactions \DIFdelbeginFL \DIFdelFL{support }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{supporting }\DIFaddendFL higher-level aspects of cognitive
...
reviews~\DIFdelbegin %DIFDELCMD < \citep[e.g.,][]{DemeEtal19, Turk13, LuriEtal18, FongEtal19,
%DIFDELCMD <   ParkEtal18b, PretEtal17, RoyEtal19, LiegEtal19, ZouEtal19,
%DIFDELCMD <   ChanGlov10, GonzEtal19}%%%
\DIFdelend \DIFaddbegin \citep[e.g.,][]{DemeEtal19, Turk13, LuriEtal18, FongEtal19,
  ParkEtal18b, PretEtal17, MannEtal18, RoyEtal19, LiegEtal19, ZouEtal19,
  ChanGlov10, GonzEtal19}\DIFaddend .  Our study extends this line of work by
finding cognitively relevant \textit{higher-order} network dynamics
that reflect ongoing cognition.  Our findings \DIFaddbegin \DIFadd{also }\DIFaddend complement other work
...
regions integrate information relatively slowly~\citep{BaldEtal17,
  ...
  ChieHone19}.  \DIFaddbegin \DIFadd{A similar hierarchy appears to play a role in
predicting future events~}\citep{LeeEtal20}\DIFadd{.  }\DIFaddend Other related work in
...
structures. \DIFaddbegin \DIFadd{Importantly, our analyses do not speak to the
...
rely on sliding windows\DIFaddbegin \DIFadd{~}\citep[e.g., as in][]{MannEtal18}\DIFaddend , and that
...
a range of parameter values and \DIFdelbegin \DIFdel{then
}\DIFdelend identifying which results were robust
to specific parameter choices. \DIFdelbegin %DIFDELCMD < 
...
data provide a window into the neural basis of cognition.  }\DIFdelend By showing that high-level cognition is
...
\DIFdelbegin \DIFdel{$\mathcal{O}(TK^{2n})$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{O}(TK^{2^n})$ }\DIFaddend space and therefore becomes intractable
...
\DIFadd{The central mathematical operations in our procedure are two bookkeeping
