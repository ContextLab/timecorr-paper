...
%DIF DEL old.tex    Fri Nov 13 16:02:29 2020
%DIF ADD main.tex   Fri Nov 13 17:16:49 2020
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{hyperref}
\usepackage{setspace}
...
\begin{abstract}
  ...
  different subgraphs of the brain's \DIFaddbegin \DIFadd{functional }\DIFaddend connectome that display
  homologous lower-level dynamic correlations.  We tested the
  hypothesis that high-level cognition is \DIFdelbegin \DIFdel{supported by }\DIFdelend \DIFaddbegin \DIFadd{reflected in }\DIFaddend high-order
  ...
  human participants either listened to a ten-minute story \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend listened
  to a temporally scrambled version of the story\DIFdelbegin \DIFdel{, or underwent a
  resting state scan}\DIFdelend .  We trained
  ...
  story \DIFdelbegin \DIFdel{or during the resting
  state scan }\DIFdelend yielded the best performance when they were trained using
  ...
  are \DIFdelbegin \DIFdel{supported
  by }\DIFdelend \DIFaddbegin \DIFadd{reflected in }\DIFaddend higher-order patterns of dynamic network
...
interactions between brain structures~\DIFdelbegin %DIFDELCMD < \citep{SporHone06, BassEtal06,
%DIFDELCMD <   Turk13, DemeEtal19, SoloEtal19, LuriEtal18, PretEtal17, ZouEtal19,
%DIFDELCMD <   MackEtal17}%%%
...
brain structures;][]{MannEtal18, FongEtal19, LuriEtal18, PretEtal17,
  ZouEtal19, DemeEtal19}.  \DIFdelbegin \DIFdel{We wondered what }\DIFdelend \DIFaddbegin \DIFadd{What might }\DIFaddend the future of this line of work
\DIFdelbegin \DIFdel{might hold.  }\DIFdelend \DIFaddbegin \DIFadd{hold?  }\DIFaddend For example, is the neural code \DIFdelbegin \DIFdel{mediated by
}\DIFdelend \DIFaddbegin \DIFadd{implemented through
}\DIFaddend higher-order interactions between brain structures~\citep[e.g.,
...
\DIFaddbegin \DIFadd{One reason one might expect to see homologous networks in a dataset is
related to the notion that network dynamics reflect ongoing neural
computations or cognitive processing~}\citep[e.g.,][]{BeatEtal16}\DIFadd{.  If
...
scan~\citep{SimoEtal16}.  \DIFaddbegin \DIFadd{Temporal scrambling has been used in a
...
\mbox{%DIFAUXCMD
\cite{HassEtal08} }\hspace{0pt}%DIFAUXCMD
argues that when brain areas are sensitive to fine
...
\DIFaddend We used a subset of the \DIFaddbegin \DIFadd{story listening and rest }\DIFaddend data to train
across-participant classifiers to decode listening times \DIFaddbegin \DIFadd{(of groups of
participants) }\DIFaddend using a blend of neural features (comprising neural
...
resting state data, only \DIFaddbegin \DIFadd{(relatively) }\DIFaddend lower-order dynamic patterns
...
\DIFadd{We sought to understand whether high-level cognition is reflected in
...
data (see }\textit{\DIFadd{Dynamic inter-subject functional connectivity
  (DISFC)}} \DIFadd{and }\textit{\DIFadd{Dynamic higher-order correlations}}\DIFadd{).  We
evaluated the efficacy of this framework at recovering known patterns
in several synthetic datasets (see }\textit{\DIFadd{Synthetic data: simulating
  dynamic first-order correlations}} \DIFadd{and }\textit{\DIFadd{Synthetic data:
  simulating dynamic higher-order correlations}}\DIFadd{).  We then
...
state scan (see }\textit{\DIFadd{Functional neuroimaging data collected during
  story listening}}\DIFadd{).  We used the relative decoding accuracies of
...
\subsection*{\DIFadd{Recovering known dynamic correlations from synthetic
  data}}
\subsubsection*{\DIFadd{Recovering dynamic first-order correlations}}
\DIFadd{We generated synthetic datasets that differed in how the underlying first-order
...
\begin{figure}[tp]
  ...
  \caption{\textbf{\DIFaddFL{Recovering known dynamic first-order correlations
      from synthetic data.}}  \DIFaddFL{Each panel displays the average
    ...
    see }\textit{\DIFaddFL{Synthetic data: simulating dynamic first-order
      correlations}}\DIFaddFL{.  }\textbf{\DIFaddFL{a. Constant correlations.}}  \DIFaddFL{These
    datasets have a stable (unchanging) underlying correlation matrix.
    }\textbf{\DIFaddFL{b. Random correlations.}}  \DIFaddFL{These datasets are generated
    using a new independently drawn correlation matrix at each new
    timepoint.  }\textbf{\DIFaddFL{c. Ramping correlations.}}  \DIFaddFL{These datasets are
    ...
    correlations with the }\textit{\DIFaddFL{first}} \DIFaddFL{timepoint's correlation
    matrix.  The right panel compares the recovered correlations with
    the }\textit{\DIFaddFL{last}} \DIFaddFL{timepoint's correlation matrix.
    }\textbf{\DIFaddFL{d. Event-based correlations.}}  \DIFaddFL{These datasets are each
...
\DIFadd{In general, wide monotonic kernel shapes (Laplace, Gaussian), and
...
\DIFadd{When the correlation dynamics were unstructured in time
...
\subsubsection*{\DIFadd{Recovering dynamic higher-order correlations}}
\DIFadd{Following our approach to evaluating our ability to recover known
...
known dynamic first-order }\textit{\DIFadd{and}} \DIFadd{second-order correlations (see
}\textit{\DIFadd{Synthetic data: simulating dynamic higher-order
  correlations}}\DIFadd{).  We generated a total of 40 datasets that varied in
...
\DIFadd{Overall, we found that we could reliably recover both first-order and
...
that when the correlations were particularly }\textit{\DIFadd{un}}\DIFadd{stable (random
...
\begin{figure}[tp]
  ...
  \caption{\textbf{\DIFaddFL{Recovery of simulated first-order and second-order dynamic correlations.}}  \DIFaddFL{Each panel displays the average correlations
    ...
    see }\textit{\DIFaddFL{Synthetic data: simulating dynamic higher-order
      correlations}}\DIFaddFL{.  All results in this figure used a Laplace kernel
    (width~$=20$).  }\textbf{\DIFaddFL{a. Constant correlations.}}
    \DIFaddFL{These first and second order dynamic correlation were
    simulated from datasets have a stable (unchanging) underlying correlation
    matrix.  }\textbf{\DIFaddFL{b. Random correlations.}}  \DIFaddFL{These first and second order dynamic correlation were
    simulated from datasets are
    generated using a new independently drawn correlation matrix at
    each new timepoint.  }\textbf{\DIFaddFL{c. Ramping correlations.}}  \DIFaddFL{These first and second order dynamic correlation were
    simulated from
  ...
  simulated correlation first and second order correlation matrices.   }\textbf{\DIFaddFL{d. Event-based correlations.}}  \DIFaddFL{These first and second order dynamic correlation were
    simulated from datasets are
    each generated using five randomly drawn correlation matrices that
    each remain stable for a fifth of the total timecourse.  The
    vertical lines denote event boundaries.}}
  \label{fig:higher_order_sims}
\end{figure}

\DIFadd{Taken together, our explorations using synthetic data indicated that
...
\subsection*{\DIFadd{Cognitively relevant dynamic high-order correlations in
  fMRI data}}
\DIFadd{We used across-participant temporal decoders to identify cognitively
relevant neural patterns in fMRI data (see }\textit{\DIFadd{Forward inference
  and decoding accuracy}}\DIFadd{).  The dataset we examined~}\citep[collected
by][]{SimoEtal16} \DIFadd{comprised four experimental conditions that exposed
participants to stimuli that varied systematically in how cognitively
engaging they were.  The }\textit{\DIFadd{intact}} \DIFadd{experimental condition had
participants listen to an audio recording of a 10-minute story.  The
}\textit{\DIFadd{paragraph}}\DIFadd{-scrambled experimental condition had participants
...
the same order.  The }\textit{\DIFadd{word}}\DIFadd{-scrambled experimental condition
...
the same order.  Finally, in a }\textit{\DIFadd{rest}} \DIFadd{experimental condition,
...
\begin{figure}[tp]
  ...
  \caption{\textbf{\DIFaddFL{Across-participant timepoint decoding accuracy varies with
      correlation order and cognitive engagement.}}
    \textbf{\DIFaddFL{a.~Decoding accuracy as a function of order: PCA.}}
    \textit{\DIFaddFL{Order}} \DIFaddFL{($x$-axis) refers to the maximum order of dynamic
    correlations that were available to the classifiers (see
    }\textit{\DIFaddFL{Feature weighting and testing}}\DIFaddFL{).  The reported
    across-participant decoding accuracies are averaged over all
    kernel shapes and widths (see }\textit{\DIFaddFL{Identifying robust decoding
      results}}\DIFaddFL{).  The $y$-values are displayed relative to chance
    ...
    }\textbf{\DIFaddFL{b.~Normalized timepoint decoding accuracy as a function of order:
      PCA.}}  \DIFaddFL{This panel displays the same results as Panel a, but here
    ...
    a lower-dimensional space.  }\textbf{\DIFaddFL{c.~Timepoint decoding accuracy as a
      function of order: eigenvector centrality.}} \DIFaddFL{This panel is in the
    ...
    }\textbf{\DIFaddFL{d.~Normalized timepoint decoding accuracy as a function of order:
      eigenvector centrality.}} \DIFaddFL{This panel is in the same format as
...
\DIFadd{In brief, we computed timeseries of dynamic high-order correlations
...
$n^\mathrm{th}$-order dynamic correlations (see }\textit{\DIFadd{Feature
  weighting and testing}}\DIFadd{, Fig.~\ref{fig:pipeline}).  We repeated this process for
...
robust to specific kernel parameters (see }\textit{\DIFadd{Identifying robust
  decoding results}}\DIFadd{).
}

\DIFadd{Our approach to estimating dynamic high-order correlations entails
...
\caption{\textbf{\DIFaddFL{Statistical summary of decoding accuracies for
    different neural features.}}  \DIFaddFL{Each column of matrices displays decoding
  ...
  a decoder that included all feature types (see }\textit{\DIFaddFL{Feature
    weighting and testing}}\DIFaddFL{).}}
\label{fig:ttests}
\end{figure}

\DIFadd{Both sets of temporal decoding analyses yielded qualitatively similar
...
\begin{figure}[tp]
  ...
  \caption{\textbf{\DIFaddFL{Top terms associated with the endpoints of the
      strongest correlations.}}  \DIFaddFL{Each color corresponds to one order of
    ...
    and projected onto the cortical surface~}\citep{CombEtal19}\DIFaddFL{.  The
    lists of terms on the right display the top five Neurosynth
    terms~}\citep{RubiEtal17} \DIFaddFL{decoded from the corresponding brain maps
    for each order.  Each row displays data from a different
    experimental condition.  Additional maps and their corresponding
    Neurosynth terms may be found in the }\textit{\DIFaddFL{Supplementary
      materials}} \DIFaddFL{(intact: Fig.~\intact; paragraph: Fig.~\para; word:
    Fig.~\word; rest: Fig.~\rest).}}
  \label{fig:neurosynth}
\end{figure}

\DIFadd{Having established that patterns of high-order correlations are
...
experimental condition.  We then used Neurosynth~}\citep{RubiEtal17} \DIFadd{to
compute the terms most highly associated with the most strongly
correlated pairs of regions in each of these matrices
(Fig.~\ref{fig:neurosynth}; see }\textit{\DIFadd{Reverse inference}}\DIFadd{).
}

\DIFadd{For all of the story listening conditions (intact, paragraph, and
...
\DIFadd{We tested the hypothesis that high-level cognition is reflected in
high-order brain network dynamics~}\citep[e.g., see][]{SoloEtal19,
  ReimEtal17}\DIFadd{.  We examined high-order network dynamics in functional
...
\begin{figure}[tp]
  ...
  \caption{\textbf{\DIFaddFL{Proposed high-order network dynamics underlying
      high-level cognition during story listening.}}  \DIFaddFL{Schematic depicts
    higher orders of
...
\DIFadd{The notion that cognition is reflected in (and possibly mediated by)
...
reviews~}\citep[e.g.,][]{DemeEtal19, Turk13, LuriEtal18, FongEtal19,
  ...
  ChanGlov10, GonzEtal19}\DIFadd{.  Our study extends this line of work by
finding cognitively relevant }\textit{\DIFadd{higher-order}} \DIFadd{network dynamics
...
reconfigure during cognition~}\citep[e.g.,][]{BassEtal06, ZhenEtal19,
  McInJirs19, TokeSomm19, SizeEtal18, ReimEtal17, BetzEtal19}\DIFadd{.
}

\DIFadd{An open question not addressed by our study pertains to how different
...
regions integrate information relatively slowly~}\citep{BaldEtal17,
  ...
  ChieHone19}\DIFadd{.  A similar hierarchy appears to play a role in
predicting future events~}\citep{LeeEtal20}\DIFadd{.  Other
...
areas~}\citep{FallEtal19}\DIFadd{.  Further study is needed to understand the
...
\DIFadd{Another potential limitation of our approach relates to recent work
suggesting that the brain undergoes rapid state changes, for example
across event boundaries~}\citep[e.g.,][]{BaldEtal17}\DIFadd{.
\mbox{%DIFAUXCMD
\cite{ShapEtal19} }\hspace{0pt}%DIFAUXCMD
used hidden semi-Markov models to estimate
state-specific network dynamics~}\citep[also see][]{VidaEtal18}\DIFadd{.  Our
...
\DIFadd{Identifying high-order network dynamics associated with high-level
...
inter-subject functional connectivity~}\citep{SimoEtal16} \DIFadd{to a dynamic
measure of inter-subject functional connectivity (DISFC) that does not
rely on sliding windows~}\citep[e.g., as in][]{MannEtal18}\DIFadd{, and that
...
\subsection*{\DIFadd{Concluding remarks}}
\DIFadd{Methods for characterizing the dynamics of high-order correlations in
...
\textit{inter-subject functional connectivity}~\DIFdelbegin %DIFDELCMD < \citep[ISFC;
%DIFDELCMD < ][]{SimoEtal16} %%%
...
Approximation and Projection~\DIFdelbegin %DIFDELCMD < \citep[UMAP;][]{McInHeal18}%%%
...
like without using dimensionality reduction to maintain \DIFdelbegin \DIFdel{scalability}\DIFdelend \DIFaddbegin \DIFadd{tractability}\DIFaddend ).
...
network comprises nodes $A$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $B$, \DIFdelbegin \DIFdel{and $C$}\DIFdelend \DIFaddbegin \DIFadd{along with several other nodes}\DIFaddend .  If $A$ and $B$ exhibit
uncorrelated activity patterns, \DIFaddbegin \DIFadd{then by definition }\DIFaddend the functional connection
(correlation) between them will be \DIFdelbegin \DIFdel{(by definition) }\DIFdelend close to 0.
However, if $A$ and $B$ each interact with \DIFdelbegin \DIFdel{$C$ }\DIFdelend \DIFaddbegin \textit{\DIFadd{other}} \DIFadd{nodes }\DIFaddend in similar ways, we
might attempt to capture those similarities \DIFdelbegin \DIFdel{using a measure that
reflects how }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend $A$\DIFaddbegin \DIFadd{'s }\DIFaddend and $B$\DIFdelbegin \DIFdel{interact with }\textit{\DIFdel{other}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{'s
interactions with those other }\DIFaddend members of the network.
...
timeseries occupies \DIFdelbegin \DIFdel{$\mathcal{O}(TK^{2n})$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{O}(TK^{2^n})$ }\DIFaddend space.  However, when we
...
exhibited different temporal properties. We \DIFaddbegin \DIFadd{also simulated
higher-order correlations and tested how well
Equation~\ref{eqn:timecorr} could recover these correlations using the
best kernel from the previous synthetic data analyses.  We then }\DIFaddend applied our
...
\subsubsection*{Synthetic data\DIFaddbegin \DIFadd{: simulating dynamic first-order correlations}\DIFaddend }
...
of dynamic \DIFaddbegin \DIFadd{first-order }\DIFaddend correlations (i.e., 10 datasets reflecting each type of
...
$\mathbf{\Sigma}_m$:
\begin{align}
   \DIFaddbegin \DIFadd{\mathbf{\Sigma}_m }&\DIFadd{= }\DIFaddend \mathbf{C}
                                          \DIFaddbegin \DIFadd{\mathbf{C}^\top\mathrm{,~where}}\label{eqn:randcorr}\\
  \DIFadd{\mathbf{C}}\DIFaddend (i, j) &\sim \mathcal{N}(0, 1)\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <   %%%
\DIFdel{\mathbf{\Sigma}_m }%DIFDELCMD < &%%%
\DIFdel{= \mathbf{C}
                                          \mathbf{C}^\top~\mathrm{, where}}%DIFDELCMD < \label{eqn:randcorr}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \DIFadd{\mathrm{,~and~where}
  }\DIFaddend \end{align}
...
the correlation dynamics are structured \DIFaddbegin \DIFadd{and exhibit first-order
autocorrelations (as in the constant datasets)}\DIFaddend .
...
datasets were intended to simulate a system that \DIFdelbegin \DIFdel{undergoes }\DIFdelend \DIFaddbegin \DIFadd{exhibits periods of
stability punctuated by }\DIFaddend occasional abrupt state changes.

\DIFaddbegin \subsubsection*{\DIFadd{Synthetic data: simulating dynamic high-order
  correlations}}

\DIFadd{We developed an iterative procedure for constructing timeseries data
...
\DIFadd{The central mathematical operations in our procedure are two
...
$n - 1$ template (from the same timepoint) as follows:
  ...
  \DIFadd{\mathbf{\Sigma_{n}}(t) }&\DIFadd{= \mathrm{mat}(\mathrm{vec}(\mathbf{\Sigma_{n-1}}(t)) \otimes \mathrm{vec}(\mathbf{\Sigma_{n-1}}(t))^\top).\label{eqn:veccorr}
}\end{align}

\DIFadd{Given a timeseries of order $n$ correlation matrices, we can draw an
order $n-1$ correlation matrix for each timepoint $t$ using
}\begin{align}
 \DIFadd{\mathbf{\sigma_{n-1}}(t) }&\DIFadd{\sim \mathcal{N}\left(\mathbf{0}, \mathbf{\Sigma_n}(t)
                   \right)\label{eqn:highorder-gen1}}\\
  \DIFadd{\mathbf{\Sigma_{n-1}}(t) }&\DIFadd{= \mathrm{mat}(\mathbf{\sigma_{n-1}}(t)).\label{eqn:highorder-gen2}
  }\end{align}
  \DIFadd{We can then use repeated applications of
  Equations~\ref{eqn:highorder-gen1} and~\ref{eqn:highorder-gen2} in
  order to obtain a synthetic dataset.
}

 \DIFadd{When the template first-order correlations are constructed to exhibit different
 ...
 \DIFadd{Following our approach to generating synthetic data exhibiting known
...
\DIFaddend \subsubsection*{Functional neuroimaging data collected during story
  listening}
...
which stimulus timepoint matched each timepoint's neural pattern\DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{(Fig.~\ref{fig:pipeline}.  }\DIFaddend We
first divided the participants into two groups: a template group,
$\mathcal{G}_{\mathrm{template}}$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(i.e., training data), }\DIFaddend and a to-be-decoded group,
$\mathcal{G}_{\mathrm{decode}}$ \DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{(i.e., test data).  }\DIFaddend We used Equation~\ref{eqn:disfc} to
...
\DIFaddbegin \begin{figure}[tp]
  ...
  \caption{\textbf{\DIFaddFL{Decoding analysis pipeline.  a. Computing dynamic
      correlations from timeseries data.}}  \DIFaddFL{Given a timeseries of
    ...
    function kernel (lower row of the panel).    }\textbf{\DIFaddFL{b. Projecting
      dynamic correlations into a lower-dimensional space.}}  \DIFaddFL{We
    ...
    }\textbf{\DIFaddFL{c. Kernel trick.}} \DIFaddFL{We project the dynamic correlations
    ...
    the next-highest order.  }\textbf{\DIFaddFL{d. Decoding analysis.}} \DIFaddFL{We split
...
\DIFdel{).  We
evaluated the efficacy of this framework at recovering known patterns
in several synthetic datasets (see }\textit{\DIFdel{Synthetic data}}%DIFAUXCMD
\DIFdel{).  We then
applied the framework to a public fMRI dataset collected as
...
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\textwidth]{figs/synthetic_data}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Recovering known dynamic correlations from
      synthetic data.}}  %DIFAUXCMD
\DIFdelFL{Each panel displays the average correlations
    ...
    the shading within each color family denotes the kernel width
    parameter.  For a complete description of each synthetic dataset,
    see }\textit{\DIFdelFL{Synthetic data}}%DIFAUXCMD
\DIFdelFL{.  }\textbf{\DIFdelFL{a. Constant correlations.}}
    %DIFAUXCMD
\DIFdelFL{These datasets have a stable (unchanging) underlying correlation
    matrix.  }\textbf{\DIFdelFL{b. Random correlations.}}  %DIFAUXCMD
\DIFdelFL{These datasets are
    generated using a new independently drawn correlation matrix at
    each new timepoint.  }\textbf{\DIFdelFL{c. Ramping correlations.}}  %DIFAUXCMD
\DIFdelFL{These
    datasets are generated by smoothly varying the underlying
    correlations between the randomly drawn correlation matrices at
    ...
    the recovered correlations with the }\textit{\DIFdelFL{first}} %DIFAUXCMD
\DIFdelFL{timepoint's
    correlation matrix.  The right panel compares the recovered
    correlations with the }\textit{\DIFdelFL{last}} %DIFAUXCMD
\DIFdelFL{timepoint's correlation
    matrix.  }\textbf{\DIFdelFL{d. Event-based correlations.}}  %DIFAUXCMD
\DIFdelFL{These datasets are
    each generated using five randomly drawn correlation matrices that
...
\DIFdel{).  The dataset we examined~}%DIFDELCMD < \citep[collected
%DIFDELCMD < by][]{SimoEtal16} %%%
\DIFdel{comprised four experimental conditions that exposed
participants to stimuli that varied systematically in how cognitively
engaging they were.  The }\textit{\DIFdel{intact}} %DIFAUXCMD
\DIFdel{experimental condition had
participants listen to an audio recording of a 10-minute story.  The
}\textit{\DIFdel{paragraph}}%DIFAUXCMD
\DIFdel{-scrambled experimental condition had participants
listen to a temporally scrambled version of the story, where the
paragraphs occurred out of order (but where the same total set of
paragraphs were presented over the full listening interval).  All
participants in this condition experienced the scrambled paragraphs in
the same order.  The }\textit{\DIFdel{word}}%DIFAUXCMD
\DIFdel{-scrambled experimental condition
had participants listen to a temporally scrambled version of the story
where the words in the story occurred in a random order.  All
participants in the word condition experienced the scrambled words in
the same order.  Finally, in a }\textit{\DIFdel{rest}} %DIFAUXCMD
...
%DIFDELCMD < \begin{figure}[tp]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\textwidth]{figs/decode_level}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\textbf{\DIFdelFL{a.~Decoding accuracy as a function of order: PCA.}}
    %DIFAUXCMD
\textit{\DIFdelFL{Order}} %DIFAUXCMD
\DIFdelFL{($x$-axis) refers to the maximum order of dynamic
    correlations that were available to the classifiers (see
    }\textit{\DIFdelFL{Feature weighting and testing}}%DIFAUXCMD
...
%DIFDELCMD < \begin{figure}[tp]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=1\textwidth]{figs/stats_heatmaps}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Statistical summary of decoding accuracies for
    different neural features.}}  %DIFAUXCMD
...
%DIFDELCMD < \begin{figure}[tp]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\textwidth]{figs/most_abs}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Top terms associated with the endpoints of the
      strongest correlations.}}  %DIFAUXCMD
\DIFdelFL{Each color corresponds to one order of
    inter-subject functional correlations. The inflated brain plots
    display the locations of the endpoints of the 10 strongest
    (absolute value) correlations at each order, thresholded at 0.999,
    and projected onto the cortical surface~}%DIFDELCMD < \citep{CombEtal19}%%%
\DIFdelFL{.  The
    lists of terms on the right display the top five Neurosynth
    terms~}%DIFDELCMD < \citep{RubiEtal17} %%%
...
\DIFdelFL{(intact: Fig.~\intact; paragraph: Fig.~\para; word:
    Fig.~\word; rest: Fig.~\rest).}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:neurosynth}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
of 50 (Fig.~\ref{fig:kernels}d).  We used this kernel to compute a
single $K$ by $K$ $n^\mathrm{th}$-order DISFC matrix for each
experimental condition.  We then used Neurosynth~}%DIFDELCMD < \citep{RubiEtal17} %%%
\DIFdel{to
compute the terms most highly associated with the most strongly
correlated pairs of regions in each of these matrices
(Fig.~\ref{fig:neurosynth}; see }\textit{\DIFdel{Reverse inference}}%DIFAUXCMD
\DIFdel{).
}%DIFDELCMD < 
...
high-order brain network dynamics~}%DIFDELCMD < \citep[e.g., see][]{SoloEtal19,
%DIFDELCMD <   ReimEtal17}%%%
...
%DIFDELCMD < \begin{figure}[tp]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=0.6\textwidth]{figs/discussion}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Proposed high-order network dynamics underlying
      high-level cognition during story listening.}}  %DIFAUXCMD
\DIFdelFL{Higher orders of
    ...
    interactions.}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:discussion}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
patterns of first-order network dynamics has been suggested by or
proposed in myriad empirical studies and
reviews~}%DIFDELCMD < \citep[e.g.,][]{DemeEtal19, Turk13, LuriEtal18, FongEtal19,
%DIFDELCMD <   ParkEtal18b, PretEtal17, RoyEtal19, LiegEtal19, ZouEtal19,
%DIFDELCMD <   ChanGlov10, GonzEtal19}%%%
\DIFdel{.  Our study extends this line of work by
finding cognitively relevant }\textit{\DIFdel{higher-order}} %DIFAUXCMD
\DIFdel{network dynamics
that reflect ongoing cognition.  Our findings complement other work
that uses graph theory and topology to characterize how brain networks
reconfigure during cognition~}%DIFDELCMD < \citep[e.g.,][]{BassEtal06, ZhenEtal19,
%DIFDELCMD <   McInJirs19, TokeSomm19, SizeEtal18, ReimEtal17, BetzEtal19}%%%
\DIFdel{.
}%DIFDELCMD < 
...
process incoming information at similar timescales.  Low-level sensory
areas integrate information relatively quickly, whereas higher-level
regions integrate information relatively slowly~}%DIFDELCMD < \citep{BaldEtal17,
%DIFDELCMD <   HassEtal08, HassEtal15, HoneEtal12a, LernEtal11, LernEtal14, ChieHone19}%%%
\DIFdel{.  Other
related work in human and mouse brains indicates that the temporal
response profile of a given brain structure may relate to how strongly
connected that structure is with other brain
areas~}%DIFDELCMD < \citep{FallEtal19}%%%
...
suggesting that the brain undergoes rapid state changes, for example
across event boundaries~}%DIFDELCMD < \citep[e.g.,][]{BaldEtal17}%%%
\DIFdel{.
\mbox{%DIFAUXCMD
\cite{ShapEtal19} }\hspace{0pt}%DIFAUXCMD
used hidden semi-Markov models to estimate
state-specific network dynamics~}%DIFDELCMD < \citep[also see][]{VidaEtal18}%%%
\DIFdel{.  Our
general approach might be extended by considering putative state
transitions. For example, rather than weighting all timepoints using a
similar kernel (Eqn.~\ref{eqn:timecorr}), the kernel function could
...
cognition required several important methods advances.  First, we used
kernel-based dynamic correlations to extended the notion of (static)
inter-subject functional connectivity~}%DIFDELCMD < \citep{SimoEtal16} %%%
\DIFdel{to a dynamic
measure of inter-subject functional connectivity (DISFC) that does not
