...
%DIF < \newcommand{\pca}{S7}
%DIF -------
\newcommand{\timeheatmap}{S3} %DIF > 
\newcommand{\features}{S4} %DIF > 
\newcommand{\intact}{S5} %DIF > 
\newcommand{\para}{S6} %DIF > 
\newcommand{\word}{S7} %DIF > 
\newcommand{\rest}{S8} %DIF > 
\newcommand{\pca}{S9} %DIF > 
%DIF -------
...
interactions between brain structures~\DIFdelbegin %DIFDELCMD < \citep{Gros88, Fris00, SporHone06, BassEtal06,
...
\DIFdelend \DIFaddbegin \citep{Gros88, Fris00, SporHone06, BassEtal06,
  ...
  MackEtal17, BresKels01, McIn00}\DIFaddend .
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Recovering known dynamic first-order correlations
    ...
    correlation matrix.  (The averages are taken across \DIFdelbeginFL \DIFdelFL{10 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{100 }\DIFaddendFL different
    randomly generated synthetic datasets of \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{each }\DIFaddendFL given category\DIFaddbeginFL \DIFaddFL{, each
    with $K = 50$ features and $T = 300$ timepoints}\DIFaddendFL .)
...
correlations are independent \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend the correlations at every other
...
\textit{Synthetic data: simulating dynamic higher-order
  correlations}).  We generated a total of \DIFdelbegin \DIFdel{40 }\DIFdelend \DIFaddbegin \DIFadd{400 }\DIFaddend datasets that varied in
...
second-order correlations.  \DIFdelbegin \DIFdel{We expected that this would happen, given
that }\DIFdelend \DIFaddbegin \DIFadd{This is because }\DIFaddend errors in our \DIFdelbegin \DIFdel{estimation }\DIFdelend \DIFaddbegin \textit{\DIFadd{estimation}}
...
correlations).  \DIFdelbegin \DIFdel{Interestingly, we also found
that }\DIFdelend \DIFaddbegin \DIFadd{Conversely, }\DIFaddend when the correlations were particularly
\textit{un}stable (random datasets), we better recovered second-order
correlations.  \DIFaddbegin \DIFadd{This is because noise in our }\textit{\DIFadd{data generation}}
\DIFadd{procedure propagates from higher orders to lower orders (see
}\textit{\DIFadd{Synthetic data: simulating dynamic high-order correlations}}\DIFadd{).
}\DIFaddend 
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Recovery of simulated first-order and second-order dynamic correlations.}  Each panel displays the average correlations
    ...
    timepoint and for each synthetic dataset.  (The averages are taken across \DIFdelbeginFL \DIFdelFL{10 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{100 }\DIFaddendFL different
    randomly generated synthetic datasets of \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{each }\DIFaddendFL given category\DIFaddbeginFL \DIFaddFL{, each
    with $K = 10$ features and $T = 300$ timepoints}\DIFaddendFL .)
    Error ribbons denote 95\% confidence intervals (taken across
    ...
    (width~$=20$).  \DIFdelbeginFL \textbf{\DIFdelFL{a. Constant correlations.}}  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Constant.}}  \DIFaddendFL These datasets
    have stable (unchanging) underlying second-order correlation
    matrices.  \DIFdelbeginFL \textbf{\DIFdelFL{b. Random correlations.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Random.}} \DIFaddendFL These datasets are
    generated using a new independently drawn second-order correlation
    matrix at each timepoint.  \DIFdelbeginFL \textbf{\DIFdelFL{c. Ramping correlations.}}
    %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Ramping.}}
    \DIFaddendFL These datasets are generated by smoothly varying the underlying
    second-order correlations between the randomly drawn correlation
    matrices at the first and last timepoints.  \DIFdelbeginFL \textbf{\DIFdelFL{d. Event-based
      correlations.}}  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Event.}}  \DIFaddendFL These datasets are each generated using five
    randomly drawn second-order correlation matrices that each remain
    stable for a fifth of the total timecourse.  The
    vertical lines denote event boundaries.  \DIFaddbeginFL \DIFaddFL{Note that the ``dips''
    ...
    boundaries.}\DIFaddendFL }
  \label{fig:higher_order_sims}
\end{figure}


\DIFaddbegin \DIFadd{We also examined the impact of the data duration (Fig.~\timeheatmap)
...
\DIFaddend Taken together, our explorations using synthetic data indicated that
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Across-participant timepoint decoding accuracy varies with
    ...
    each curve has been normalized to \DIFdelbeginFL \DIFdelFL{have a maximum value of 1 and a
    minimum value of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{be bounded between }\DIFaddendFL 0 \DIFaddbeginFL \DIFaddFL{and 1
    }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{including }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inclusive) by subtracting }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{upper }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{minimum accuracy
    (across all folds }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{lower bounds of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{orders) and then dividing by }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{respective 95\% confidence intervals}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{maximum
    accuracy (again, across all folds and orders}\DIFaddendFL ).  Panels a and b used PCA to
...
%DIFDELCMD <   ParkEtal18b, PretEtal17, MannEtal18, RoyEtal19, LiegEtal19, ZouEtal19,
...
\DIFdelend \DIFaddbegin \citep[e.g.,][]{DemeEtal19, Turk13, LuriEtal18, FongEtal19,
  ...
  ChanGlov10, GonzEtal19, McIn00, BresKels01}\DIFaddend .  Our study extends this line of work by
...
\DIFaddbegin \DIFadd{One limitation of our approach relates to how noise propagates in our
...
conditions we examined~}\citep{SimoEtal16}\DIFadd{.  Therefore a caveat to our
...
\DIFaddend Another potential limitation of our approach relates to recent work
...
We constructed a total of \DIFdelbegin \DIFdel{40 }\DIFdelend \DIFaddbegin \DIFadd{400 }\DIFaddend different multivariate timeseries,
collectively reflecting a total of 4 qualitatively different patterns
of dynamic first-order correlations (i.e., \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{100 }\DIFaddend datasets reflecting each type of
...
static.  We constructed \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{100 }\DIFaddend multivariate timeseries whose observations
...
\begin{align}
  ...
  covariance matrix, and the \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{100 }\DIFaddend datasets each used a different
  covariance matrix.

  \paragraph*{Random.}  We generated a second set of \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{100 }\DIFaddend synthetic
  ...
  \paragraph*{Ramping.}  We generated a third set of \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{100 }\DIFaddend synthetic
...
\paragraph*{Event.} We generated a fourth set of \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{100 }\DIFaddend synthetic datasets
...
The central mathematical \DIFdelbegin \DIFdel{operations }\DIFdelend \DIFaddbegin \DIFadd{operation }\DIFaddend in our procedure \DIFdelbegin \DIFdel{are two bookkeeping
functions, $\mathrm{vec}(\cdot)$ and $\mathrm{mat}(\cdot)$.  The $\mathrm{vec}(\cdot)$ function takes as input }\DIFdelend \DIFaddbegin \DIFadd{is the Kronecker
product ($\otimes$).  The Kronecker product of }\DIFaddend a $K \times K$ \DIFdelbegin \DIFdel{symmetric
matrixand returns as output a $\left(\frac{K^2 - K}{2} + K\right)$-dimensional column vector
containing }\DIFdelend \DIFaddbegin \DIFadd{matrix,
$m_1$, with itself (i.e., $m_1 \otimes m_1$) produces a new
$K^2 \times K^2$ matrix, $m_2$ whose entries reflect a scaled tiling
of }\DIFaddend the entries in \DIFdelbegin \DIFdel{the upper triangle and diagonal.  The
$\mathrm{mat}(\cdot)$ function inverts $\mathrm{vec}(\cdot)$ by taking
as input a $\left(\frac{K^2 - K}{2} + K\right)$-dimensional columnvector and returning a $K \times K$ symmetric matrix as output.
We
can then generate an order }\DIFdelend \DIFaddbegin \DIFadd{$m_1$.  If these tilings (scaled copies of $m_1$) are
...
first-order timeseries of dynamic correlations (see }\textit{\DIFadd{Synthetic
  data: simulating dynamic first-order correlations}}\DIFadd{), we use this
procedure (applied independently at each timepoint) to transform it
into a timeseries of }\DIFaddend $n$\DIFdelbegin \DIFdel{correlation matrix (for one timepoint, $t$)from an order
$n - 1$ template (from the same timepoint) as follows:
}\begin{align*}
  \DIFdel{\mathbf{\Sigma_{n}}(t) }&\DIFdel{= \mathrm{mat}(\mathrm{vec}(\mathbf{\Sigma_{n-1}}(t)) \otimes \mathrm{vec}(\mathbf{\Sigma_{n-1}}(t))^\top).%DIFDELCMD < \label{eqn:veccorr}%%%
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{\textsuperscript{th}-order correlations.  When
...
Given a timeseries of \DIFdelbegin \DIFdel{order }\DIFdelend $n$\DIFdelbegin \DIFdel{correlation matrices, we can draw }\DIFdelend \DIFaddbegin \DIFadd{\textsuperscript{th}-order correlations, we
...
at a given timepoint is $m_n$, then we can generate }\DIFaddend an order $n-1$
correlation matrix \DIFdelbegin \DIFdel{for each timepoint $t$ using
}\begin{align*}
 \DIFdel{\mathbf{\sigma_{n-1}}(t) }&\DIFdel{\sim \mathcal{N}\left(\mathbf{0}, \mathbf{\Sigma_n}(t)
                   \right)\label{eqn:highorder-gen1}}\\
  \DIFdel{\mathbf{\Sigma_{n-1}}(t) }&\DIFdel{= \mathrm{mat}(\mathbf{\sigma_{n-1}}(t)).%DIFDELCMD < \label{eqn:highorder-gen2}%%%
  }\end{align*}%DIFAUXCMD
%DIFDELCMD <   %%%
...
specified }\DIFaddend high-order correlations \DIFdelbegin \DIFdel{and synthetic data will exhibit the same category of
 temporal profile}\DIFdelend \DIFaddbegin \DIFadd{at orders 1 through $n$}\DIFaddend .  Following
our approach to generating synthetic data exhibiting known first-order
correlations, we constructed a total of \DIFdelbegin \DIFdel{40 }\DIFdelend \DIFaddbegin \DIFadd{400 }\DIFaddend additional multivariate
timeseries, collectively reflecting a total of 4 qualitatively
different patterns of dynamic correlations (i.e., \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{100 }\DIFaddend datasets
reflecting each type of dynamic pattern: constant, random, ramping,
and event).  \DIFaddbegin \DIFadd{Each timeseries comprised 10 zero-order features (dimensions)
