...
correlations are independent from the correlations at every other
...
\textit{Synthetic data: simulating dynamic higher-order
  correlations}).  We generated a total of 400 datasets that varied in
...
second-order correlations.  This is because errors in our \textit{estimation}
...
correlations.  This is because noise in our \textit{data generation}
procedure propagates from higher orders to lower orders (see
\textit{Synthetic data: simulating dynamic high-order correlations}).
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Across-participant timepoint decoding accuracy varies with
    ...
    each curve has been normalized to be bounded between 0 and 1
    (inclusive) by subtracting the minimum accuracy
    (across all folds and orders) and then dividing by the maximum
    accuracy (again, across all folds and orders).  Panels a and b used PCA to
...
first-order timeseries of dynamic correlations (see \textit{Synthetic
