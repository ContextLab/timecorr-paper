...
paper~\citep{SimoEtal16}.

Following our prior related work, we used hierarchical topographic
factor analysis~\citep{MannEtal18} to derive a compact representation
of the data.  In brief, this approach approximates the timeseries of
voxel activations (44,415 voxels) using a much smaller number of
radial basis function nodes (in this case 700 nodes).  This provides a
convenient representation for examining full-brain network dynamics.
All of the analyses we carried out on the neuroimaging dataset were
performed in this lower-dimensional space.  Code for carrying out
hierarchical topographic factor analyses on fMRI data may be found as
part of the BrainIAK toolbox~\citep{brainiak}, which may be downloaded
at \href{https://brainiak.org/}{brainiak.org}.
...
story participants listened to).  We use decoding accuracy on held-out
data (from held-out participants) as a proxy for the extent to which
the inputted neural patterns reflected stimulus-driven cognitive
processing in a similar way across individuals.  \textbf{JRM STOPPED HERE}
...
\textbf{JRM OUTLINE STOPPED HERE}
