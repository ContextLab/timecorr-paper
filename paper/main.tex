...
\begin{figure}[tp]
  ...
  \caption{\textbf{Recovery of simulated first-order and second-order dynamic correlations.}  Each panel displays the average correlations
    ...
    (width~$=20$).  \textbf{Constant correlations.}  These datasets
...
$n - 1$ template (from the same timepoint) using the Kronecker product as follows:
\begin{align}
  \mathbf{\Sigma_{n}}(t) &= \mathrm{mat}(\mathrm{vec}(\mathbf{\Sigma_{n-1}}(t)) \otimes \mathrm{vec}(\mathbf{\Sigma_{n-1}}(t))).\label{eqn:veccorr}
\end{align}
