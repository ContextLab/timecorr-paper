...
into a row vector (this reshaped vector is $(\frac{K^2 - K}{2})$-dimensional).  Second, we apply a dimensionality reduction step
...
Because $\mathbf{X}_n$ will have the same shape as the original data
...
approximate the output of Equation~\ref{eqn:timecorr} with a $T$ by
$K$ matrix (as described next), it becomes feasible to compute even
...
by $K$ timeseries requires only $\mathcal{O}(TK^2)$ space-- the same
as would be required to compute first-order dynamic
correlations. \textbf{JRM STOPPED HERE}
