...
matrix (ramping covariance matrix Fig.~\ref{fig:synthetic_data},  C.),
we generated two random covariance
matrices, $K_{1}$ and $K_{2}$.  We then computed a covariance matrix
for each of the 300 timepoint, $K(t)$ 
by taking the linearly spaced weights ($w$) of the two random matrices, 
\begin{align}
K(t) = w(t) * K_{1} + 1 - w(t) *  K_{2}
\end{align}


(Fig.~\ref{fig:synthetic_data})
...
with the kernel and the specific structure of the data
