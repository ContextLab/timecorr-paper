...
\end{align}
and $\mathbf{Y}_n^p(t)$ denotes the correlation matrix at timepoint $t$
...
is to compute the $T$ by $M$ matrix, $\mathbf{X}$, that is closest to
the original $T$ by $N$ matrix, $\mathbf{Y}$, where (typically)
...
data are compared (and how) in order to characterize how well
...
weights of each of these $\mathbf{\Lambda}$ matrices such that the
...
to an auditorally presented story, listened to a temporally scrambled
version of the story, or underwent a resting state scan (see
\textit{Functional neuroimaging data collected during story
  listening}).  We used the relative decoding accuracies of
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Across-participant decoding accuracy varies with
    ...
    $p < 0.05$).  Figure~\ttests displays additional comparisons between
