...
copying from lucy's message on slack:
- for each order, compute the average (pairwise) correlations
- pick the 10 strongest (absolute value) correlations
- find the RBFs on the endpoints of those correlations (i.e. the features being correlated)
- make a map by summing together those (thresholded at 0.999) RBFs
- decode with neurosynth
- print out top 10 terms
