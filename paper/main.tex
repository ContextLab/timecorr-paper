...
\begin{figure}
  ...
  \caption{\textbf{Decoding by order.
      a. Relative decoding accuracy by order.} Relative to chance,
    decoding accuracy for each condition (intact, paragraph, word,
    and rest) as a function of increasingly more orders.  The red dots indicates
    ...
    condition. \textbf{c. Optimized weights.}
...
\begin{figure}
  ...
  \caption{\textbf{Inter-subject correlations by order.
      a. Inter-subject correlation heatmaps by order.}  Heatmap
    denoting how correlated each subject is with every other subject
    for each order, and grouped by condition (intact (I), paragraph
    (P), word (W),
    and rest (R)). \textbf{b. Distribution of correlations for each
      condition by order}
   Inter-subject correlations by conditon for each order.}
  \label{fig:corrmats}
\end{figure}
