...
\item \cite{LuriEtal18}: review of time-varying connectivity

\item \cite{FongEtal19}: higher variability in dynamic functional connectivity
...
\item \cite{ParkEtal18b}: dynamic causal modelling: compute causal interaction
...
\item \cite{ThomEtal18}: simulations for benchmarking time-varying connectivity
...
\item \cite{SizeEtal18}: there are common patterns of connectivity
  (graph measures) across people; potential citation to motivate
  eigenvector centrality approach

\item \cite{ReimEtal17}: they propose that the brain processes stimuli
  by forming increasingly complex functional cliques and cavities;
  good motivation for our general approach

\item \cite{GonzEtal19}: dynamic functional connectivity during rest
  is influenced by spontaneous cognitive-task-like processes.  they
  also use low-dimensional embeddings to visualize how dynamic functional
  connectivity during rest can reveal periods of externally imposed tasks

\item \cite{BetzEtal19}: during movie watching, the brain's
  connectivity profile dynamically reconfigures.  when ISFC is high,
  this corresponds to times when network modularity is low and
  connectivity between cognitive systems is high.
