...
\begin{figure}[tp]
  ...
  \caption{Neural patterns.  a. A space of neural features.
   ...
   b. Summarizing neural patterns.  To efficiently compute
...
\begin{figure}[tp]
  ...
  \caption{Recovering known dynamic first-order correlations
    ...
    see Synthetic data: simulating dynamic first-order
      correlations.  a. Constant correlations.  These
    datasets have a stable (unchanging) underlying correlation matrix.
    b. Random correlations.  These datasets are generated
    using a new independently drawn correlation matrix at each new
    timepoint. c. Ramping correlations.  These datasets are
    ...
    d. Event-based correlations.  These datasets are each
...
\begin{figure}[tp]
  ...
  \caption{Recovery of simulated first-order and second-order dynamic correlations.  Each panel displays the average correlations
    ...
    matrices at the first and last timepoints. Event.  These datasets are each generated using five
...
\begin{figure}[tp]
  ...
  \caption{Across-participant timepoint decoding accuracy varies with
      correlation order and cognitive engagement.
    a.~Decoding accuracy as a function of order: PCA.
    ...
    b.~Normalized timepoint decoding accuracy as a function of order:
    ...
    a lower-dimensional space. c.~Timepoint decoding accuracy as a
   ...
   d.~Normalized timepoint decoding accuracy as a function of order:
...
\caption{Statistical summary of decoding accuracies for
...
\begin{figure}[tp]
  ...
  \caption{Top terms associated with the most strongly
...
\begin{figure}[tp]
  ...
  \caption{Proposed high-order network dynamics underlying
...
\begin{figure}
  ...
  \caption{Estimating dynamic high-order correlations.  Given
...
\begin{figure}
  ...
  \caption{Examples of kernel functions. Each panel displays
    per-timepoint weights for a kernel centered at $t = 50$, evaluated
    at 100 timepoints ($\tau \in \left[1, ..., 100\right]$).
    a. Uniform kernel. The weights are timepoint-invariant;
   ...
   b. Dirac $\delta$ kernel. Only the observation at
    timepoint $t$ is given a non-zero weight (of 1).
    c. Gaussian kernels. Each kernel's weights fall off in
    ...
    width parameters ($\sigma^2$) are displayed.  d. Laplace
    ...
    ($b$) are displayed.  e. Mexican hat (Ricker wavelet)
      kernels.  Each kernel's weights fall off in time according to a
...
\begin{figure}[tp]
  ...
  \caption{Decoding analysis pipeline.  a. Computing dynamic
    ...
    function kernel (lower row of the panel).   b. Projecting
    ...
    c. Kernel trick. We project the dynamic correlations
    ...
    the next-highest order.  d. Decoding analysis. We split
