...
Specifically, we asked, for each timepoint: what are the correlations
between the first group's and second group's activity patterns at each
order.  We then took weighted mixture of the correlation matrices at
each order using mixing parameter, $\phi$, where $0 <\phi< 1$ reflects a
weighted mixture of order based decoding. We optimized  $\phi$, by subdividing the training group. We note that the decoding test we used is a conservative in which we count a timepoint label as incorrect if it is not an exact match.
...
activity for each group (see \textit{Materials and methods}).  For
...
parameters to obtain a weighted-mixture of each order’s correlation
...
template.) We repeated this procedure 10 times (randomly re-assigning
