...
exhibited different temporal properties. We also used this synthetic
data to simulate higher-order correlations and tested how well Equation~\ref{eqn:timecorr}
could recover these correlations using the best kernel from the
previous synthetic data analysis.  We applied our approach to a
...
\subsubsection*{Simulated higher-order correlations}
We simulated first and second order correlations reflecting the 4 qualitatively different patterns
of synthetic dynamic correlations.
 We repeated the processes 10 times for each order and for each of the 4 dynamic pattern (Constant, Random, Ramping, and
Event) explored in \textit{Synthetic data}.  First,
 we generated a timeseries of correlations which comprised 50 features (dimensions)
that varied over 300 timepoints using 1 of the 4 \textit{Synthetic data} dynamic
patterns. To calculate the next increasing order of corrrelations, ${ Y_{n+1,t}}$, we
multiplied the vectorized correlation matrix by its transpose, for
each timepoint:

\begin{align}
Y_{n=1,t} = vec(\mathbf{\Sigma}_{t})\cdot
  vec(\mathbf{\Sigma}_{t})^\top \\
  Y_{n+1,t} = vec(Y_{n,t}) \cdot vec(Y_{n,t})^\top\label{eqn:veccorr}
  \end{align}

  We repeated this processes for the number of desired orders (N) to
  generate a timeseries of $\mathbf{N^{th}}$ correlation matrices.  Then, observations were drawn at each timepoint from a  ${ Y_{N-1,t}}$-mean multivariate Gaussian distribution with a
 covariance matrix defined as ${ Y_{N+1,t}}$ and the resultant vector
 was reshaped to the matrix the size of ${ Y_{N-1,t}}$:
 \begin{align}
   { \hat{Y}_{N-1,t}} \sim vec(\mathcal{N}({ Y_{n,t}}, { Y_{N,t}}))^{-1}
   \end{align}
 This process was then repeated for all decreasing orders, in which
 observations werre drawn at each timepoint from a  ${ Y_{n,t}}$-mean
 multivariate Gaussian distribution, but the covariance matrix is
 defined at the previously estimated reshaped matrix $\hat{Y}_{n,t}$:
 \begin{align}
   { \hat{Y}_{n,t}} \sim vec(\mathcal{N}({ Y_{n,t}}, {\hat{ Y}_{n+1,t}}))^{-1}
 \end{align}
We repeat this process until $n = 1$, then we drew observations at each timepoint
from a zero-mean multivariate Gaussian distribution with a
covariance matrix defined for each timepoint as described below:

\begin{align}
 s_t \sim \mathcal{N}(\mathbf{0},  \hat{Y}_{n=1,t})
\end{align}

For our specific analyses the resulting synthetic timeseries comprised
50 features and 300 timepoints. We then used this synthetic data to test how well Equation~\ref{eqn:timecorr}
could recover the simulated first and second order correlations.  To mitigate temporal blur in
estimating each timeseries of dynamic correlations, we used a Dirac $\delta$ function
kernel (which places all of its mass over a single timepoint)
Fig.~\ref{fig:kernels}b) to compute each lower-order correlation and then used a
Laplace kernel with a width of 20 to compute  $\mathbf{ X_{n,t}}$, applying
temporal blur at just the last step.  Then, for each timepoint, we
correlated the simulated correlation matix, ${ Y_{n,t}}$ with the
Equation~\ref{eqn:timecorr} estimated correrlation matric, $\mathbf{ X_{n,t}}$.
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.7\textwidth]{figs/timecorr_pipeline}
...
\subsection*{Recovering known higher-order dynamic correlations from synthetic data}
We generated a synthetic timeseries from simulated first and second order dynamic correlation
matrices that differed in how the underlying
correlations changed over time.  For each dataset, we applied
Equation~\ref{eqn:timecorr} with a Laplace kernel with a width of 20.
We assessed how well the recovered correlations matched simulated
first and second order correlations. 
(Fig.~\ref{fig:higher_order_sims}).  For every dataset we tested,
our approach recovered the correlation dynamics we embedded into the
data.  However, the quality of these recoveries varied across
...
\begin{figure}[tp]
  ...
  \caption{\textbf{Recovery of simulated first and second order dynamic correlations.}  Each panel displays the average correlations
    between the vectorized upper triangles of the recovered
...
regions integrate information relatively slowly~\citep{BaldEtal17,
  HassEtal08, HassEtal15, HoneEtal12a, LernEtal11, LernEtal14, ChieHone19}.  Other
...
our analyses do no speak to the physiological basis of higher-order
