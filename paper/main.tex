...
\subsubsection*{Feature selection and testing}
...
$n + 1$ $\mathbf{\Lambda}$ matrices correlating timepoints from  $\mathcal{G}_{\mathrm{train}}$
and $\mathcal{G}_{\mathrm{test}}$.  


\textbf{JRM STOPPED HERE}--
say something about delta function kernel to avoid something at higher
orders, then in the last order use a gaussian, laplace, or mexican hat kernel.

compute lambda, then compute optimal weighted mix using half of the
template data, then decode the to-be-decoded data using those weights.

From below: quasi-Newton method of Broyden, Fletcher, Goldfarb, and Shanno (BFGS
~\citep{NoceWrig06}) for optimization. We repeated this
cross-validation process 10 times for each parameter set.

\subsubsection*{Identifying robust decoding results}
Params: Gaussian, Laplace, mexican hat kernels.  Widths of 5, 10, 20,
and 50.
