...
matrix (Ramping, Fig.~\ref{fig:synthetic_data},  C.),
...
Gaussian distribution centered on $K(t)$. We also explored
recovery by comparing this ramping synthetic data to only the first random covariance matrix $K_{1}$
(First, Fig.~\ref{fig:synthetic_data},  C.) and using only the last
random covariance matrix $K_{2}$ (Last, Fig.~\ref{fig:synthetic_data},
C.). 

For the synthetic data containing block structure, we followed the same
...
after each 60 consecutive timepoints.  We then pieced the blocks together
to create a sythetic dataset with 300 total timepoints but drawn from
5 separate covariance matrices. We then compared recovery using the
total dataset (All, Fig.~\ref{fig:synthetic_data},  D.) or limiting to
the block specific covariance matrix (Block 1-5, Fig.~\ref{fig:synthetic_data},  D.).
...
\begin{figure}
  ...
  \caption{\textbf{Dynamic correlation recovery with synthetic data. } Using synthetic data containing different
   ...
   we test how well we can recover using two kernels (delta and gaussian,
    width = 10).  We plot recovery using of datasets containing the
